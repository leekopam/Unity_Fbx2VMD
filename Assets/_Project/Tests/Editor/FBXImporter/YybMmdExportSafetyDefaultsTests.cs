    ...
    public class YybMmdExportSafetyDefaultsTests
    {
            ...
            float evaluatorXzTargetMagnitude)
        {
            ...
            MethodInfo method = runnerType.GetMethod(
                ...
                BindingFlags.Static | BindingFlags.NonPublic,
                binder: null,
                types: new[] { typeof(FileManager), typeof(bool), typeof(float), typeof(float), typeof(float), typeof(float), typeof(float), typeof(float), typeof(bool), typeof(float) },
                modifiers: null);

            Assert.That(method, Is.Not.Null, "YYB runner must support runtime-only evaluator-basis right foot X/Z probes.");

            return (bool)method.Invoke(null, new object[] { manager, enabled, weight, maxOffset, positiveZScale, toesBlendWeight, frameGateStart, frameGateEnd, evaluatorXzReferenceEnabled, evaluatorXzTargetMagnitude });
        }
