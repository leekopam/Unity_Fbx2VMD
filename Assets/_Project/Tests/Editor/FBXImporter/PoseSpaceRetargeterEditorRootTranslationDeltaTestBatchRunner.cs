...
{
    public static class PoseSpaceRetargeterEditorRootTranslationDeltaTestBatchRunner
    {
        public static void Run()
        {
            ...
            {
                resultPath = Path.Combine(Directory.GetCurrentDirectory(), "TestResults-PoseSpaceRetargeterEditorRootTranslationDelta.xml");
            }

            DateTimeOffset start = DateTimeOffset.UtcNow;
            var results = new List<TestResultRecord>();
            var tests = new PoseSpaceRetargeterEditorRootTranslationDeltaTests();
            ...
            Console.WriteLine($"PoseSpaceRetargeterEditorRootTranslationDelta tests completed; results written to {resultPath}");
            EditorApplication.Exit(failed == 0 ? 0 : 1);
        }

        private static void RunTest(List<TestResultRecord> results, string methodName, TestDelegate action)
        {
            string name = typeof(PoseSpaceRetargeterEditorRootTranslationDeltaTests).FullName + "." + methodName;
            DateTimeOffset start = DateTimeOffset.UtcNow;
            string failure = null;
        ...
        private static string BuildXml(IReadOnlyList<TestResultRecord> results, double duration)
        {
            ...
            writer.AppendLine($"  <test-suite type=\"TestFixture\" name=\"{SecurityElement.Escape(typeof(PoseSpaceRetargeterEditorRootTranslationDeltaTests).FullName)}\" result=\"{runResult}\" total=\"{results.Count}\" passed=\"{passed}\" failed=\"{failed}\">");
