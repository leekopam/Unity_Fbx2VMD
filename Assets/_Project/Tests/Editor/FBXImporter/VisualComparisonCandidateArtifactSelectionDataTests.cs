    ...
    public class VisualComparisonCandidateArtifactSelectionDataTests
    {
        ...
        public void Given_RunnerSelection_When_Serializing_Then_PreservesArtifactContract()
        {
            ...
            Type runnerType = runtimeAssembly.GetType(
                ...
                throwOnError: true);
            Type selectionType = runnerType.GetNestedType(
                "SummaryCandidateArtifactSelection",
                BindingFlags.Public | BindingFlags.NonPublic);
            Assert.That(selectionType, Is.Not.Null);
            Assert.That(selectionType.BaseType, Is.EqualTo(dataType));

            object selection = Activator.CreateInstance(selectionType, nonPublic: true);
            selectionType.GetField("selected_candidate_role").SetValue(selection, "evaluation_candidate_metrics");
            selectionType.GetField("selected_candidate_is_acceptance_artifact").SetValue(selection, true);
