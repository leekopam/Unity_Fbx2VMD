...
{
    public static class PoseSpaceRetargeterLowestFootBottomTestBatchRunner
    {
        public static void Run()
        {
            ...
            {
                resultPath = Path.Combine(Directory.GetCurrentDirectory(), "TestResults-PoseSpaceRetargeterLowestFootBottom.xml");
            }

            DateTimeOffset start = DateTimeOffset.UtcNow;
            var results = new List<TestResultRecord>();
            var tests = new PoseSpaceRetargeterLowestFootBottomTests();

            RunTest(results, nameof(tests.Given_FiniteFootHeight_When_CalculatingFootBottom_Then_SubtractsRadius),
                tests.Given_FiniteFootHeight_When_CalculatingFootBottom_Then_SubtractsRadius);
            RunTest(results, nameof(tests.Given_NonFiniteFootBottom_When_CalculatingFootBottom_Then_ReturnsFalse),
                tests.Given_NonFiniteFootBottom_When_CalculatingFootBottom_Then_ReturnsFalse);
            RunTest(results, nameof(tests.Given_FiniteFootHeights_When_CalculatingLowestFootBottom_Then_UsesLowerFootMinusRadius),
                tests.Given_FiniteFootHeights_When_CalculatingLowestFootBottom_Then_UsesLowerFootMinusRadius);
            RunTest(results, nameof(tests.Given_NonFiniteFootBottom_When_CalculatingLowestFootBottom_Then_ReturnsFalse),
                tests.Given_NonFiniteFootBottom_When_CalculatingLowestFootBottom_Then_ReturnsFalse);
            ...
            Console.WriteLine($"PoseSpaceRetargeterLowestFootBottom tests completed; results written to {resultPath}");
            EditorApplication.Exit(failed == 0 ? 0 : 1);
        }

        private static void RunTest(List<TestResultRecord> results, string methodName, TestDelegate action)
        {
            string name = typeof(PoseSpaceRetargeterLowestFootBottomTests).FullName + "." + methodName;
            DateTimeOffset start = DateTimeOffset.UtcNow;
            string failure = null;
        ...
        private static string BuildXml(IReadOnlyList<TestResultRecord> results, double duration)
        {
            ...
            writer.AppendLine($"  <test-suite type=\"TestFixture\" name=\"{SecurityElement.Escape(typeof(PoseSpaceRetargeterLowestFootBottomTests).FullName)}\" result=\"{runResult}\" total=\"{results.Count}\" passed=\"{passed}\" failed=\"{failed}\">");
