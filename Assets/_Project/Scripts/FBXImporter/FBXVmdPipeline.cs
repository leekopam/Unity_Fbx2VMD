...
namespace Fbx2Vmd.FBXImporter
{
    public class FBXVmdPipeline : MonoBehaviour
    {
        ...
        [Range(0f, 1f)] public float preSetHumanPoseRightEndpointPositionReferenceWeight = 1f;

        [Tooltip("Maximum per-frame pre-SetHumanPose right-foot endpoint X/Z correction distance.")]
        [Range(0f, 0.2f)] public float preSetHumanPoseRightEndpointPositionReferenceMaxOffset = 0.025f;

        [Tooltip("Scale applied only to positive world-Z endpoint correction before SetHumanPose; 1 keeps existing behavior.")]
        [Range(0f, 1f)] public float preSetHumanPoseRightEndpointPositionReferencePositiveZScale = 1f;

        [Tooltip("Blend from foot-only endpoint delta to the foot/toes average before SetHumanPose.")]
        [Range(0f, 1f)] public float preSetHumanPoseRightEndpointPositionReferenceToesBlendWeight = 1f;
        ...
        [Range(0f, 6000f)] public float preSetHumanPoseRightEndpointPositionReferenceFrameGateStart = 0f;
            ...
            float recordingPlaybackSpeedOverride = float.NaN)
        {
            if (_isProcessing)
            {
                Debug.LogWarning("[FBXVmdPipeline] 다른 FBX 처리가 진행 중이라 smoke 진단을 시작하지 않았습니다.");
                return false;
            }

            if (string.IsNullOrWhiteSpace(fbxFileName))
            {
                Debug.LogError("[FBXVmdPipeline] smoke 진단 FBX 파일명이 비어 있습니다.");
                return false;
            }
            ...
            {
                Debug.LogError($"[FBXVmdPipeline] smoke 진단 FBX를 찾을 수 없습니다: {sourcePath}");
                return false;
            }
            ...
            Debug.Log(
                $"[FBXVmdPipeline] Editor smoke 진단 시작: FBX={Path.GetFileName(sourcePath)}, " +
            ...
            {
                Debug.Log($"[FBXVmdPipeline] Editor smoke capture override: {_editorSmokeCaptureWidth}x{_editorSmokeCaptureHeight}");
            }
        ...
        {
            Debug.Log("[FBXVmdPipeline] Editor smoke batch advance reset: target idle 상태를 다음 FBX 시작 전 다시 고정합니다.");
        ...
        private bool HandleLoadFromImportFolderClick()
        {
            ...
            string selectedFile = fbxFiles[0];
            Debug.Log($"[FBXVmdPipeline] Import_FBX 폴더에서 로드: {Path.GetFileName(selectedFile)}");
            ProcessFBXAsync(selectedFile);
            return true;
        }
        #endregion
            ...
            try
            {
                ...
                if (_shouldSaveToImportFolder)
                {
                    ...
                    if (isNewFile || File.Exists(targetPath))
                    {
                         ...
                         {
                             Debug.Log($"[FBXVmdPipeline] Controlled Import_FBX importer preserved: {targetPath}");
                         }
                    }
#endif
                }
                ...
                Debug.Log($"[FBXVmdPipeline] FBX 로드 시작: {targetPath}");
                ...
                if (importedModel != null)
                {
                    ...
                    // FBXVmdPipeline 자신(this)을 넘겨서 설정을 공유함
        ...
        internal async Task<bool> ProcessFBXSessionAsync(string sourcePath)
        {
            ...
            try
            {
                ...
                {
                    Debug.Log($"[FBXVmdPipeline] FBX 임포트/Unity 촬영 전용 모드: {outputBaseName} (VMD 자동 녹화 생략)");
                }
                ...
                {
                    Debug.LogWarning("[FBXVmdPipeline] Ghost Humanoid Avatar 생성 실패. Auto bone mapping으로 재시도합니다.");
        ...
        private void TryCopyVmdToAdditionalFolder(VmdSaveResult result)
        {
            ...
            try
            {
                ...
                Debug.Log($"[FBXVmdPipeline] VMD 추가 복사: {targetPath}");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[FBXVmdPipeline] VMD 추가 복사 실패: {ex.Message}");
            }
        }
        ...
        private VmdSaveResult ApplyEditorSmokeThumbRiskFailure(VmdSaveResult result, MotionComparisonProbe probe)
        {
            ...
            Debug.LogWarning($"[FBXVmdPipeline] Editor smoke diagnostic only: {diagnosticMessage}");
            return result;
#else
            return result;
#endif
        ...
        {
            Debug.LogWarning($"[FBXVmdPipeline] {errorMessage}");
        ...
        private bool ValidateBoneMapping(Dictionary<string, string> boneMapping)
        {
            ...
            {
                Debug.LogError("[FBXVmdPipeline] BoneMapping_Data.txt가 비어 있습니다.");
                return false;
            }
            ...
            foreach (string boneName in requiredBones)
            {
                ...
                {
                    Debug.LogError($"[FBXVmdPipeline] 필수 본 매핑 누락: {boneName}");
                    valid = false;
                }
            }
            ...
            {
                Debug.LogWarning($"[FBXVmdPipeline] Chest 매핑이 문서 기준과 다릅니다. 현재값: {chestBone}, 기대값: Skeleton_Spine1");
            }
            ...
            if (showBoneMappingLog)
            {
                ...
                {
                    Debug.Log($"[FBXVmdPipeline] BoneMapping: {mapping.Key} -> {mapping.Value}");
                }
            }
        ...
        private bool ValidateGhostAvatar(GameObject importedModel)
        {
            ...
            {
                Debug.LogError("[FBXVmdPipeline] Ghost Animator 또는 Avatar가 없습니다.");
                return false;
            }

            if (!ghostAnimator.avatar.isValid || !ghostAnimator.avatar.isHuman)
            {
                Debug.LogError($"[FBXVmdPipeline] Ghost Avatar가 유효하지 않습니다. valid={ghostAnimator.avatar.isValid}, human={ghostAnimator.avatar.isHuman}");
                return false;
            }
        ...
        private AnimationClip ExtractPrimaryClip(Animation ghostAnim)
        {
            ...
            {
                Debug.LogError($"[FBXVmdPipeline] 애니메이션 길이가 올바르지 않습니다: {targetClip.length}");
                return null;
            }

            if (targetClip.length > 1000f)
            {
                Debug.LogWarning("[FBXVmdPipeline] 애니메이션 길이가 비정상적으로 깁니다. Assimp timeScale을 확인하세요.");
            }

            if (showRuntimeAnimationLog)
            {
                Debug.Log($"[FBXVmdPipeline] Clip: {targetClip.name}, Length: {targetClip.length:F3}s, FrameRate: {targetClip.frameRate}");
            }
        ...
        private void PrepareTargetCharacter(GameObject targetObject, Animator targetAnimator, Animator ghostAnimator)
        {
            ...
            {
                CameraFacingController.FaceTargetToCamera(targetObject);
            }
            else
            {
                targetObject.transform.rotation = Quaternion.identity;
            ...
            if (ikControl != null)
            {
                Debug.Log("[FBXVmdPipeline] 자동 retarget 경로에서 IKControl을 제거합니다.");
                Destroy(ikControl);
            }
        ...
        private void ConfigureFinalIkFootGroundingExperiment(GameObject targetObject)
        {
            ...
            {
                Debug.Log(
                    $"[FBXVmdPipeline] Final IK foot grounding experiment configured: " +
        ...
        private void DisableMmdPostPoseCorrectionForRetarget(GameObject targetObject)
        {
            ...
            if (changed > 0)
            {
                Debug.Log($"[FBXVmdPipeline] Retarget 중 MMD4Mecanim 어깨 PPH 보정 {changed}개를 일시 비활성화했습니다.");
            }
        }
        ...
        internal void FailSession(string message, Exception exception = null)
        {
            if (exception != null)
            {
                Debug.LogError($"[FBXVmdPipeline] {message}\n{exception}");
            }
            else
            {
                Debug.LogError($"[FBXVmdPipeline] {message}");
        ...
        private void LogRetargetPlaybackStabilitySummary()
        {
            ...
            Debug.Log(
                $"[FBXVmdPipeline] Retarget playback stability: " +
        ...
        private void LogEditorSmokeThumbState(string stage)
        {
            ...
            Debug.Log(
                $"[FBXVmdPipeline] Editor smoke thumb state ({stage}): " +
        ...
        private IEnumerator StartRecordingSequence(
            ...
            string outputBaseName = "")
        {
            Debug.Log($"[FBXVmdPipeline]  녹화 시작까지 {startDelay}초 대기 중...");
            ...
            if (ghostAnim != null && clip != null)
            {
                ghostAnim.clip = clip;
                ghostAnim.Play();
                Debug.Log($"[FBXVmdPipeline]  Ghost 애니메이션 재생 시작: {clip.name}");
            }
            ...
            if (recorderController != null)
            {
                ...
                recorderController.StartAutoRecording(
                    ...
                    overwriteExistingOutput: true);
                Debug.Log($"[FBXVmdPipeline]  VMD 녹화 동시 시작! (길이: {clipLen:F2}초)");
            }
            else
            {
                Debug.LogWarning("[FBXVmdPipeline]  HumanoidSampleCode 컴포넌트가 Target에 없습니다. 녹화 건너뜀.");
            }
        }
        ...
        private Dictionary<string, string> LoadBoneMappingRuntime()
        {
            ...
            {
                Debug.Log($"[FBXVmdPipeline] BoneMapping 로드 성공 (Resources/{loadName})");
            ...
            {
                Debug.LogWarning($"[FBXVmdPipeline] BoneMapping 로드 실패: Resources/{loadName}.txt (자동 본 매핑으로 폴백합니다.)");
            }
        ...
        private void ConfigureEditorHumanoidMuscleReference(PoseSpaceRetargeter retargeter, string importedFilePath, string sourceFilePath)
        {
            ...
            {
                Debug.LogWarning($"[FBXVmdPipeline] Unity Editor Humanoid 기준 클립을 찾지 못했습니다: {relativePath}");
                return;
            }
        ...
        private void ConfigureEditorManualFingerPoseReference(PoseSpaceRetargeter retargeter, AnimationClip referenceClip)
        {
            ...
            {
                Debug.LogWarning("[FBXVmdPipeline] 수동 기준 손가락 Reference prefab/controller를 찾지 못해 raw FBX finger curve를 사용합니다.");
                return;
            }
