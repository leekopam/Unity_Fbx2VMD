...
namespace Fbx2Vmd.FBXImporter
{
    ...
    public static class YybVisualComparisonRequestWatcher
    {
        ...
        private static void Poll()
        {
            ...
            YybVisualComparisonRequestOptionsMapper.RequestData request;
            try
            {
                Debug.Log("[YybVisualComparisonRequestWatcher] 요청 감지됨");
                request = JsonUtility.FromJson<YybVisualComparisonRequestOptionsMapper.RequestData>(
            ...
            try
            {
                YybVisualComparisonBatchRunner.RunWithOptions(
                    YybVisualComparisonRequestOptionsMapper.Map(request));
            }
            catch (Exception ex)
            {
                YybVisualComparisonBatchRunner.RunCompleted -= HandleRunCompleted;
        ...
        private static void PersistRequest(YybVisualComparisonRequestOptionsMapper.RequestData request)
        {
            if (request == null)
            {
                return;
